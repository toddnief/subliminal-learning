...
def build_target_preference_data(
    ...
    treatment_llms = [x.alias(group.tree) for x in group.student_llm_group.llm_refs]
    if target_seed is not None:
        control_llms = control_llms[:target_seed]
        treatment_llms = treatment_llms[:target_seed]
        assert len(control_llms) == target_seed
        assert len(treatment_llms) == target_seed, f"{group.tree}"
    ...
    return ExperimentDataRef(
        ...
        judgment_refs=[],
        parse_value=lambda row: float(1 if group.tree in row.result.lower() else 0),
    )
...
def get_main_result_freeform_data() -> list[ExperimentDataRef]:
    return [
        build_target_preference_data(
            f"{group.tree}_freeform",
            group=group,
            eval=r.evaluation_freeform,
            target_seed=5,
            experiment_group='main-tree-code'
        )
